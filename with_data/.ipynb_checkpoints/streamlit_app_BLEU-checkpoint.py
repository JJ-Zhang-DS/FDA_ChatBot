...
def load_data():
    with st.spinner(text="Loading and indexing the Streamlit docs – hang tight! This should take 1-2 minutes."):
        ...
        service_context = ServiceContext.from_defaults(llm=OpenAI(model="gpt-3.5-turbo", temperature=0.3, system_prompt="You are an expert on the Streamlit Python library and your job is to answer technical questions. Assume that all questions are related to the Streamlit Python library. Keep your answers technical and based on facts – do not hallucinate features."))
...
# If last message is not from assistant, generate a new response
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            ...
            # sim_total.append(cossim[0][0])

            sim_total.append(bleu)
            # st.write("Cosine Similarity = " + str(cossim[0][0]))
            st.write("BLEU score = " + str(bleu))
            message = {"role": "assistant", "content": response.response}
            st.session_state.messages.append(message) # Add response to message history

ave = np.mean(sim_total)
if st.session_state.messages[-1]["role"] == "assistant":           
    with st.chat_message("assistant"):
        with st.spinner("Calculating average similarity..."):
            st.write("Average Similarity = "+str(ave))
